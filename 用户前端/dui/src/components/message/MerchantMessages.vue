<template>
  <div class="message-list-page">
    ...
    <div class="list-header">
      ...
      <span class="header-title">商家消息</span>
      ...
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh" success-text="刷新成功">
        ...
        >
          ...
          <van-empty
            ...
            image="search"
            :description="searchKeyword ? '没有找到匹配的消息' : '暂无商家消息'"
          />
        </van-list>
      </van-pull-refresh>
    </div>
    ...
    >
      <div class="detail-popup" v-if="detailMessage">
        ...
        <div class="detail-meta">
          <van-icon name="shop-o" size="16" color="#7232dd" />
          <span class="detail-type">商家消息</span>
...
const msgType: MessageType = 'merchant'
...
async function handleReadAll() {
  try {
    await showDialog({ title: '提示', message: '确定将所有商家消息标记为已读吗？', showCancelButton: true })
  } catch { return }
...
function goMerchantChat(msg: Message) {
  showDetail.value = false
  if (msg.merchantId) {
    router.push(`/message/chat/${msg.merchantId}`)
  }
}
...
.msg-link { font-size: 12px; color: #7232dd; cursor: pointer; }
