...
const autoLogin = async () => {
  try {
    ...
    if (loginInfo.expireTime && loginInfo.expireTime > now) {
      ...
      if (result.code === 200) {
        sessionStorage.setItem('token', result.token || 'user-token')
        showToast('自动登录成功')
...
const onLogin = async () => {
  ...
  try {
    ...
    console.error('后端登录接口调用失败，使用模拟登录:', error)
    // 模拟登录：测试账号 test001/test002，密码 123456
    handleMockLogin()
  }
}
...
const handleMockLogin = () => {
  ...
  const testAccounts = [
    ...
    { username: 'user0001', password: '123456' }
  ]
...
const fetchCaptcha = async () => {
  try {
    ...
    console.error('获取验证码失败，使用本地模拟验证码:', error)
    // 使用本地生成的模拟验证码
    generateMockCaptcha()
  }
}
...
const generateMockCaptcha = () => {
  ...
  // 生成模拟的 Captcha-Key
  const mockKey = 'mock-captcha-key-' + Date.now()
  captchaKey.value = mockKey
  localStorage.setItem('captchaKey', mockKey)
  localStorage.setItem('mockCaptcha', captchaText)
  ...
  console.log('生成模拟验证码:', captchaText)
}
